...
## [2026-02-17] Database Connection String Fix

**Issue:** SQLite database connection error due to malformed path in connection string

**Problem:** Connection string in `Umbraco13/appsettings.json` used incorrect path format:
```json
"DefaultConnection": "Data Source=.//AppData//funds.db"
```

**Fix Applied:** Updated to correct path format:
```json
"DefaultConnection": "Data Source=AppData/funds.db"
```

**Changes:**
- Removed unnecessary leading `./` from path
- Changed double slashes `//` to single slashes `/`
- Used forward slashes (cross-platform compatible)

**Files Modified:**
- `Umbraco13/appsettings.json` (line 9)

**Result:** Database connection path now correctly resolves to `AppData/funds.db` relative to application root
